...
export class VNCClient extends EventEmitter {
  ...
  /** Track whether we've received rect updates since last request */
  ...
  /**
   * ── Perf Opt #4: Adaptive frame wait ──
   * Instead of fixed 800ms delay, poll for rect receipt with 50ms intervals.
   * Falls back to maxWaitMs if no rects arrive (e.g., static screen).
   */
  private async waitForFrameUpdate(maxWaitMs = 800): Promise<void> {
    ...
    if (this.pendingRectReceived) {
      // Got rect — wait a tiny bit more for any trailing rects
      await this.delay(50);
    }
  }
  ...
  async captureScreen(): Promise<ScreenFrame> {
    ...
    // ── Perf Opt #4: Adaptive wait instead of fixed 800ms ──
